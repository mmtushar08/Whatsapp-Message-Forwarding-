...
function parseBody(body: Record<string, unknown>): ForwardingRuleInput {
  ...
  return {
    ...
    extraRecipients: parseCSV(extraRecipients as string[] | string | undefined).map(
      normalizePhoneNumber,
    ),
    keywordFilters: parseCSV(keywordFilters as string[] | string | undefined),
    allowedSenders: parseCSV(allowedSenders as string[] | string | undefined).map(
      normalizePhoneNumber,
    ),
...
export function createRuleHandler(req: Request, res: Response): void {
  ...
  if (
    limits.maxAdditionalRules !== -1 &&
    countRulesForWorkspace(wsId) >= limits.maxAdditionalRules
  ) {
    res
      .status(402)
      .json({ error: 'Upgrade to Pro for multiple forwarding rules.', requiredPlan: 'pro' });
    return;
  }
  ...
  try {
    res
      .status(201)
      .json({ rule: createRule(wsId, parseBody(req.body as Record<string, unknown>)) });
  } catch (e) {
